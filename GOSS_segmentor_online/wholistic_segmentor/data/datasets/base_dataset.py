...
class BaseDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.img_pixelmap_list is not None:
            label = self.read_label(self.img_pixelmap_list[index], self.label_dtype)
        else:
            label = None
        raw_label = label.copy()

        # load raw semantic label
        if self.raw_label_transform is not None:
            raw_label = self.raw_label_transform(raw_label, self.seg_list[index])['semantic']
        if not self.is_train:
            # do not save this during training
        ...
        # save prediction for official evaluation.
        name = os.path.splitext(os.path.basename(self.img_pixelmap_list[index]))[0]
        ...
        if self.target_transform is not None:
            label_dict = self.target_transform(label, self.seg_list[index])
