...
def save_annotation(label,
    ...
    if image_itself is not None:
        colored_label = 0.5 * colored_label + 0.5 * image_itself
        colored_label[np.where(image==255)] = 255
...
def save_instance_annotation(label,
    ...
    if image is not None:
        # colored_label = 0.5 * colored_label + 0.5 * image
        colored_label = 0.5 * colored_label + 1.5*image
