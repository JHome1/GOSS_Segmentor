...
class PQStatCat():
        ...
        def __iadd__(self, pq_stat_cat):
            ...
            self.iou_cluster += pq_stat_cat.iou_cluster
            self.iou_num     += pq_stat_cat.iou_num
...
class PQStat():
    ...
    def pq_average(self, categories, is_unknown):
        pq, sq, rq, n = 0, 0, 0, 0
        per_class_results = {}
        ...
        for label, label_info in categories.items():
            ...
            if is_unknown is not None:
                cat_is_unknown = label_info['is_unknown'] == 1
                if is_unknown != cat_is_unknown:
                    continue
            ...
            iou_cluster = iou_cluster + self.pq_per_cat[label].iou_cluster
            iou_num     = iou_num + self.pq_per_cat[label].iou_num
            ...
            if label == 172:
                print('TP/FP/FN is {}/{}/{} for the unknown class.'.format(tp, fp, fn))

        print('mIoU for pure clustering is {:.3f}'.format(iou_cluster/iou_num))
        return {'pq': pq / n, 'sq': sq / n, 'rq': rq / n, 'n': n}, per_class_results


@get_traceback
def pq_compute_single_core(proc_id, annotation_set, gt_folder, pred_folder, categories, 
                           target_transform, data_loader, config):
    pq_stat = PQStat()
    ...
    for gt_ann, pred_ann in annotation_set:
        ...
        pan_gt   = target_transform(pan_gt, gt_ann['segments_info'])['wholistic_id'].numpy()
        ...
        for label_tuple, intersection in gt_pred_map.items():
            ...
            if iou >= 0:
                pq_stat[gt_segms[gt_label]['category_id']].iou_num     += 1       
                pq_stat[gt_segms[gt_label]['category_id']].iou_cluster += iou
...
def pq_compute(gt_json_file, pred_json_file, gt_folder=None, pred_folder=None, 
               target_transform=None, data_loader=None, config=None):
    ...
    print("Evaluation wholistic segmentation metrics:")
    print("Ground truth:")
    ...
    metrics = [("All", None), ("Known Classes", False), ("UnKnown Class", True)]
    results = {}
    for name, is_unknown in metrics:
        results[name], per_class_results = pq_stat.pq_average(categories, is_unknown=is_unknown)
        if name == 'All':
            results['per_class'] = per_class_results
    print("{:10s}| {:>5s}  {:>5s}  {:>5s} {:>5s}".format("", "PQ", "SQ", "RQ", "N"))
    print("-" * (10 + 7 * 4))

    for name, _is_unknown in metrics:
